...
- `src/components/preview/PreviewToolbar.tsx` is one client component holding the Share link control ("this version" vs "always latest") and the diagnostics pill. It is **portalled into `<div id="preview-topbar-slot">`** (declared first in `src/app/layout.tsx`, so the bar sits above the site nav at the very top). Panels expand **inside the bar** and push the page down — nothing is `fixed`/`sticky` (both strand at the document bottom inside the CMS preview iframe) and nothing floats over the content. `AudienceSwitcher` is hidden on `/preview` (`DemoToolbar`).
