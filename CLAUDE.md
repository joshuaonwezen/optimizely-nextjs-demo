...
**Experimentation and personalization are documented across four pages** (split 2026-09-24, previously two): `/demo/personalization` is a thin hub that only compares the four paths and links out - keep it thin. `/demo/feature-experimentation` owns the middleware `__v_` mechanism, FX native attributes and ODP-as-an-FX-audience. `/demo/web-experimentation` owns both WX delivery modes. `/demo/odp` owns profiles, segments, the direct-to-Graph path and the live Settings panel. Each page carries its own personalization framing, since personalization is a configuration of each product rather than a fourth product. Adding a demo route needs one entry in `src/lib/getDemoLinks.ts` (do not invent a new category - `src/app/demo/page.tsx` keys three `Record`s off the label); the sidebar table of contents is derived from `section[id]` + the first `<h2>` inside it, so that pairing is the only convention to keep.
...
- **Two delivery modes, chosen in the demo Settings panel** (`WX_MODE_KEY` in localStorage, default soft-nav). Soft-nav holds the region and navigates to `__v_wx--*`; pre-paint renders base **and** the variant in one response and flips `data-cms-variation` on `<html>` before first paint so CSS reveals the right one - no request, no hold, no URL change. The mode lives in localStorage rather than a cookie because a cookie cannot change a statically prerendered page; the consequence is that the server sends both subtrees in both modes.
- **Never emit a per-page `<style>` next to the rendered content.** Doing it for the reveal CSS added a sibling the client tree did not have in the same place, shifting alignment by one and producing a hydration mismatch (server `<a data-component="ProductCardBlock">` vs a client `Column` `<div>`). The reveal rules are therefore a static constant (`WX_REVEAL_STYLE`) in the layout `<head>`, which is why the variant wrapper is tagged with the fixed token `__variant__` plus `data-cms-variant-name` rather than the variation name - static CSS cannot match a dynamic name.
- **A hydration warning that only reproduces under `next dev` is probably HMR, not your code.** The soft-nav swap threw one on ~1 in 10 dev first loads while a production build of the same commit was clean over 40 loads; Turbopack can serve the HTML and the client RSC payload from different compilations. Verify against `npm run build && npm start` before changing rendering code to chase it. `WxVariationSwap` still gates its work on a `useSyncExternalStore` hydration signal, which cut the dev rate ~5x and is correct regardless.
- **Hide the inactive variant subtree with `display:none`, never `visibility:hidden`.** A `visibility:hidden` element still intersects, so `AutoTracker`'s `IntersectionObserver` would fire `mb_feature_viewed` for the hidden copy. `display:none` generates no boxes.
- **Any client component inside a variation must guard with `isInactiveVariant(el)`** (`wxVariation.ts`), because both subtrees hydrate and effects run regardless of CSS. Three call sites do today: `FxBucketingEvent` (whose `decide(flagKey, [])` fires a REAL impression, so a duplicate skews the experiment), `AutoTracker` and `RecommendationBlockClient`.
- **FX pre-empts WX only when the FX variation actually exists on that page.** Do not gate on "a `__v_` segment is present" - that was a real bug. The `homepage` flag ships its persona variations (`personal`/`business`/`mortgages`/`investments`) through a **rollout** with `cms_flag: true` and **no `cms_route`**, and `routeMatches()` treats an absent `cms_route` as *every* route. So middleware appends `__v_homepage--investments` on pages that have no `investments` variation; Graph falls back to base via `includeOriginal` so FX changes nothing there, but gating on the segment silently disabled the WX bridge. Worse, `AutoTracker` writes `demo_persona` from the browsed path, so simply visiting `/investments` was enough to kill the bridge on it from the next load. The page now compares the decided variation names against the page's own (`decidedVariationApplies`). Setting `cms_route: "/"` on the `homepage` flag in the FX console is a worthwhile complementary fix, but the code no longer depends on it.
- **A variation route is the SAME page as its clean path.** Use `cleanPathname()` (`variationPath.ts`) for anything that treats a pathname change as a new pageview. Measured: without it the soft-nav swap re-fired one FX impression per hero flag and a duplicate `mb_feature_viewed`, because the swap remounts the tree and `AutoTracker` resets its dedupe set on pathname change.
