...
## Pull existing types from CMS (CLI 2.0.0)

To generate TypeScript from what's already in the CMS (e.g. types created via the UI):

```bash
OPTIMIZELY_CMS_CLIENT_ID=xxx OPTIMIZELY_CMS_CLIENT_SECRET=yyy \
  npx optimizely-cms-cli config pull --output ./src/content-types --group
```

`--group` organises files by base type (`page/`, `component/`, etc.) and co-locates display templates with their content types.
